...
fantasy_api = YahooFantasyApi(17457, 'nhl')
db = Database()
...
standings = {}
for team in db.get_standings():
    team_data = {
        'w': team[1],
        'l': team[2],
        'pf': team[3]
    }
    standings[team[0]] = team_data

remaining_matchups = []
for week in range(12, 12 + 1):
...
def simulate_season(team_variables, standings, matchups):
    ...
    for matchup in matchups:
        ...
        team_one_pf = team_variables[team_one]['avg'] + random_bm() * team_variables[team_one]['std']
        team_two_pf = team_variables[team_two]['avg'] + random_bm() * team_variables[team_two]['std']
    ...
    final_standings = dict(sorted(final_standings.items(), key=lambda x: (x[1]['w'], x[1]['pf']), reverse=True))
...
for team, result in monte_carlo(points_for, standings, remaining_matchups).items():
    ...
    if make_playoffs:
        ...
        print(f'{have_bye}\t\t{make_playoffs}\t{team}')
